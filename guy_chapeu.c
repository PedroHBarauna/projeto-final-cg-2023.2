...
{
    glPushMatrix();
    glTranslatef(0.0, 0.26, 0.0); // Ajuste da posição da coroa em relação à cabeça

    // Adiciona uma rotação de -90 graus em torno do eixo x para deitar a coroa
    glRotatef(-90.0, 1.0, 0.0, 0.0);

    glColor3f(0.0, 1.0, 0.0);          // Cor dourada para a coroa
    glutSolidTorus(0.04, 0.11, 7, 11); // Desenhar uma coroa usando um toro sólido
    glPopMatrix();

    // Adiciona a esfera acima do chapeu
    glPushMatrix();
    glTranslatef(0.0, 0.26, 0.0);  // Ajuste da posição da esfera acima do chapeu
    glColor3f(0.0, 1.0, 0.0);      // Cor vermelha para a esfera
